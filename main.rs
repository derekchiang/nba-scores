...
static ESPN_URL: &'static str = "http://espn.go.com/nba/";
...
fn main() {
    let request = RequestWriter::new(Get, from_str(ESPN_URL).unwrap());
    let mut response = match request.read_response() {
        Ok(response) => BufferedReader::new(response),
        Err(_) => unreachable!(),
    };
    not_utf8::cond.trap(|_| {~""}).inside(|| {
        for line in response.lines() {
            if line.find_str(PATTERN).is_none() { continue; }
